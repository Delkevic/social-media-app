...
func GetNotifications(c *gin.Context) {
    ...
    for i := range notifications {
        if notifications[i].FromUserID > 0 {
            ...
            database.DB.Select("id, username, profile_image").First(&fromUser, notifications[i].FromUserID)
    ...
    for _, notification := range notifications {
        ...
        if notification.PostID != nil {
            notificationResponse["postId"] = *notification.PostID
        }
        
        if notification.CommentID != nil {
            notificationResponse["commentId"] = *notification.CommentID
        }
