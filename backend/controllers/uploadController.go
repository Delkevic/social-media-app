...
// ServeImage - Yüklenen görselleri servis et
func ServeImage(c *gin.Context) {
    ...
    if os.IsNotExist(err) {
        ...
        c.JSON(http.StatusInternalServerError, Response{
            Success: false,
            Message: "Dosya bilgisi alınamadı: " + err.Error(),
        })
        return
    }
    
    fmt.Printf("Dosya bulundu: %s, boyut: %d bytes\n", imagePath, fileInfo.Size())
    ...
    if err != nil {
        ...
        c.JSON(http.StatusInternalServerError, Response{
            Success: false,
            Message: "Görsel okunamadı: " + err.Error(),
        })
        return
    }
    ...
    } else if strings.HasSuffix(imageName, ".svg") {
        contentType = "image/svg+xml"
    } else {
        // Dosya içeriğinden MIME tipini tespit et
        contentType = http.DetectContentType(fileData)
    }
    
    fmt.Printf("Dosya servis ediliyor: %s, içerik türü: %s\n", imageName, contentType)
    
    // Cache kontrolü için header'lar ekle
    ...
    fmt.Printf("Dosya başarıyla servis edildi: %s\n", imageName)
}
