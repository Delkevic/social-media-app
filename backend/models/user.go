...
// FollowRequest - Takip isteği modeli (özel hesaplar için)
...
type Notification struct {
	ID        uint      `gorm:"primaryKey"`
	UserID    uint      // Bildirim alıcısı
	FromUserID uint     // Bildirimi tetikleyen kullanıcı
	Type      string    // like, comment, follow, mention
	Content   string    // Bildirim içeriği
	PostID    *uint     // İlgili gönderi (varsa)
	CommentID *uint     // İlgili yorum (varsa)
